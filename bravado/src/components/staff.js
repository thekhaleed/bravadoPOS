...
function Staff(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://localhost:8000/staff/create/', formDataObj, {
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8000/staff/delete/${id}/`);
      props.handleFetch();
       // Call fetchStaff after deleting a customer
  ...
  const displayStaff =props.AllStaff.map(staff => {
    ...
    return(
      <tr  className='table.active table-row text-capitalize bg-danger p-0 ' key={index}>
        ...
        <td>{staff.duty}</td>
        <td>{staff.address}</td>
        <td>{staff.created.substring(0,19)}</td>
        <td className='action text-cente'>
          ...
          <svg xmlns="http://www.w3.org/2000/svg" onClick={() => handleDelete(staff.id)} width="14" height="14" fill="#dc3545" className="bi bi-trash3 table-icons m-2" viewBox="0 0 16 16">
  ...
  return (
    <div className='col-12 col-sm-11 col-md-11 text-light bg-dange'>
      <div className='p-2'>
        <h5 className='head-title'>Staff </h5>
        <hr className='line'/>
      </div>
      ...
      <div className='screen bg-dange d-flex p-0'>
        ...
        <aside className='bg-warnin p-1 side-panel'>
          <section className='ss'>
            <h5 className='form-title mb-3'>Add Staff</h5>
          </section>
          ...
          <form onSubmit={handleSubmit} className='customer-form d-flex flex-column text-center gap-3 '>
            ...
            <div className="form-floating">
              <input 
                ...
                required
              />
              <label for="duty" className='text-secondary labeler'>Role</label>
            </div>
